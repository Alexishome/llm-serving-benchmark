...
if [ -d "/workspace/vllm7-run-env" ]; then
  source /workspace/vllm7-run-env/bin/activate
elif [ -d "/workspace/vllm7-env" ]; then
  source /workspace/vllm7-env/bin/activate
elif [ -d "/workspace/vllm2-env" ]; then
  source /workspace/vllm2-env/bin/activate
else
  echo "Missing /workspace/vllm7-run-env, /workspace/vllm7-env, or /workspace/vllm2-env." >&2
  exit 1
fi
...
CONFIGS=(
  ...
  "experiments/experiment_config_remote_mixed_clinical_vllm_long_prefill_isolation_60_qwen25_7b.yaml"
)
...
cat results/remote_mixed_clinical_vllm_long_prefill_isolation_60_qwen25_7b/summary_metrics.csv
...
python3 -m control_plane.analyze_routes \
  --request-metrics results/remote_mixed_clinical_vllm_long_prefill_isolation_60_qwen25_7b/request_metrics.json \
  --markdown-output results/remote_mixed_clinical_vllm_long_prefill_isolation_60_qwen25_7b/route_summary.md
